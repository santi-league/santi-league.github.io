...
{   //variables you might actually want to change
    ...
    {   // GameMgr.Inst.record_uuid becomes populated when we have looked at a log
        e = e || window.event;
        const keyCode = typeof e.keyCode === "number" ? e.keyCode : e.key && e.key.length === 1 ? e.key.toUpperCase().charCodeAt(0) : undefined;
        if ((KEY == keyCode) && GameMgr.Inst.record_uuid)
            if (checkscene(uiscript.UI_Replay) || checkscene(uiscript.UI_Loading))
                downloadlog();
        if (BATCH_KEY == keyCode && (!REQUIRE_CTRL_FOR_BATCH || e.ctrlKey))
            showDownloadMenu();
    });

    if (typeof GM_registerMenuCommand === "function")
    {
        GM_registerMenuCommand("批量下载牌谱 (Tenhou格式)", showDownloadMenu);
    }
    ...
    //simple UI helpers to trigger batch downloads
    function showDownloadMenu()
    {
        if (document.getElementById("download-paipu-menu"))
            return;

        const menuDiv = document.createElement("div");
        menuDiv.id = "download-paipu-menu";
        ...
        menuDiv.style.boxShadow = "0 4px 20px rgba(0,0,0,0.3)";
        menuDiv.style.zIndex = "10000";
        menuDiv.innerHTML = `
            <h3 style="margin-top:0;">批量下载雀魂牌谱</h3>
            <p style="margin:8px 0;">选择输入方式：</p>
            <button id="dpn-manual" style="display:block;width:100%;margin-bottom:10px;padding:8px;">手动输入多个链接/UUID</button>
            <button id="dpn-file" style="display:block;width:100%;padding:8px;">从文本文件加载</button>
            <button id="dpn-cancel" style="display:block;width:100%;margin-top:10px;padding:8px;">取消</button>
        `;
        ...
        document.getElementById("dpn-manual").onclick = () => {
            closeMenu();
            GetPaipusJSON();
        };
        document.getElementById("dpn-file").onclick = () => {
            closeMenu();
            GetPaipusFromFile();
        };
        document.getElementById("dpn-cancel").onclick = closeMenu;
    }

    function createStatusDiv()
    {
        const statusDiv = document.createElement("div");
        statusDiv.id = "download-paipu-status";
        ...
        statusDiv.style.minWidth = "260px";
    ...
    function normalizeUuid(input)
    {
        ...
        if (uuid.includes("="))
        {
            ...
            uuid = uuidParts[0];
            if (uuidParts.length > 2 && parseInt(uuidParts[2]) === 2 && game && game.Tools && typeof game.Tools.DecodePaipuUUID === "function")
    ...
    function GetPaipusJSON(paipulinks = [])
    {
        ...
        processNextLink(uuids, 0, statusDiv);
    }
    ...
    function processNextLink(uuids, index, statusDiv)
    {
        ...
        statusDiv.innerHTML = `正在处理 ${index + 1}/${uuids.length}: ${uuid}`;
        downloadSinglePaipu(uuid, function()
        {
            setTimeout(() => processNextLink(uuids, index + 1, statusDiv), 300); //throttle to avoid bans
        }, function(errorMessage)
        {
            statusDiv.innerHTML = `处理 ${uuid} 时出错：${errorMessage || "未知错误"}`;
            setTimeout(() => processNextLink(uuids, index + 1, statusDiv), 1000);
        });
    }

    function downloadSinglePaipu(uuid, onSuccess, onError)
    {
    ...
    function fetchRecordByUuid(uuid, onSuccess, onError)
    {
        if (!uuid)
        {
        ...
        app.NetAgent.sendReq2Lobby(
            "Lobby",
            "fetchGameRecord",
            { game_uuid: uuid, client_version_string: clientVersion },
            function(err, record)
            {
                if (err || !record)
                {
                    console.error("下载牌谱失败:", uuid, err);
                    if (typeof onError === "function")
                        onError(err ? (err.msg || err) : "记录为空");
                    return;
                }
                try
                {
    ...
    function exportRecord(record)
    {
        ...
        const filename = ((new Date(record.head.end_time * 1000)).toLocaleDateString() + "_" + results["rule"]["disp"] + ".json")
            .replace(/[ \/]/g,"_");
    ...
    {
        fetchRecordByUuid(GameMgr.Inst.record_uuid);
    }
})();
// vim: ts=4  et
